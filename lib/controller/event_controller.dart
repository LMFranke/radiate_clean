...
class EventController extends ChangeNotifier {
  ...
  void init() {
    ...
    getAllEvents(false).then((value) {
      ...
      eventListSortedByDate.addAll(eventListSorted);
      notifyListeners();
    },);
  }
  ...
  void _populateEventList() {
    ...
    for (ServiceScheduledTableData serviceIndex in serviceSortedByDate) {
      ...
      SchedulesTableData schedule = eventList.where((element) => element.id == serviceIndex.serviceId,).first;
      ClientsTableData client = clientList.where((element) => element.id == schedule.clientId,).first;
      ServiceTableData service = serviceList.where((element) => element.id == serviceIndex.serviceId).first;
  ...
  Future getAllServicesScheduled(bool isNeedToNotify) async {
    await db.fetchAllServiceScheduled().then(
      (value) {
  ...
  Future addEvent(TypeOfWork isHalfDay, Frequency frequency) async {
    await _addScheduled(selectedClient!.id, frequency).then(
      (value) {

        if (_tempScheduleId != 0) {
          _addServiceScheduled(_tempScheduleId, selectedService!.id, selectedDate!, isHalfDay);
          _tempScheduleId = 0;
          getAllServicesScheduled(false);
        }
        notifyListeners();
  ...
  Future _addScheduled(int clientId, Frequency frequency) async {
    ...
    await db.insertEvent(schedule).then(
      (value) {
  ...
  Future _addServiceScheduled(int scheduledId, int serviceId, DateTime date, TypeOfWork isHalfDay) async {
    ServiceScheduledTableCompanion serviceScheduled = ServiceScheduledTableCompanion(
  ...
  void _sortServiceScheduledList() {
    serviceSortedByDate = serviceScheduledList
        .where((event) =>
            event.serviceStatus != ServiceStatus.finish.index
                && event.serviceStatus != ServiceStatus.canceled.index
                && event.date.isBefore(DateTime.now().add(const Duration(days: 7))))
        .toList()
  ...
  void sortListBySelectedDay(DateTime selectedDay) {
    ...
    for (Event eventIndex in eventListSorted) {
      ...
      if (eventIndex.serviceScheduled.date.day == selectedDay.day
          && eventIndex.serviceScheduled.date.month == selectedDay.month
          && eventIndex.serviceScheduled.date.year == selectedDay.year) {
